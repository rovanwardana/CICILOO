...
// document.getElementById('add-bill-tab').addEventListener('click', function () {
//     const tabContainer = document.querySelector('.flex.items-center.space-x-2');
//     const count = tabContainer.querySelectorAll('button[id^="bill-"]').length + 1;
//     const newTab = document.createElement('button');
//     newTab.textContent = 'Bill ' + count;
//     newTab.className = 'px-4 py-2 bg-white border-b-2 border-transparent text-gray-500 hover:text-blue-600';
//     newTab.id = 'bill-' + count;
//     tabContainer.insertBefore(newTab, document.getElementById('add-bill-tab'));
// });
...
//         user.name.toLowerCase().includes(query) && !participants.find(p => p.id === user.id)
//     );
...
//     // Tampilkan chip peserta
//     const chip = document.createElement("div");
//     chip.className = "bg-blue-100 text-blue-800 px-3 py-1 rounded-full flex items-center space-x-2";
//     chip.innerHTML = `
//         <span>${user.name}</span>
//         <button type="button" class="text-red-600 hover:text-red-800 text-sm" data-id="${user.id}">&times;</button>
//     `;
...
//         input.value = p.id;
//         selectedParticipants.appendChild(input);
//     });
// }
...
// function updateSplitSummary() {
//     const summary = document.getElementById("summary-participants");
//     summary.innerHTML = "";
//     participants.forEach(p => {
//         const li = document.createElement("li");
//         li.textContent = `${p.name} - Auto Calculated (placeholder)`;
